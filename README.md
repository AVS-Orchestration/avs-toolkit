...
1.  **The Blueprint (`VS-XXX.md`)**: The human-editable source of truth. It contains the Agile Goal, Instructions, and pointers to context. This is what you commit to Git.
2.  **The Briefcase (`VS-XXX-assembled.yaml`)**: An execution-ready snapshot. Created by `avs assemble`, it contains the logic plus the **full injected text** of all context assets.
...
* `illustrative-example/`: A complete Resume Tailoring stream.
  * `VS-001...`: Analysis & Strategy phase.
  * `VS-002...`: Tailored Drafting phase.
  * `VS-003...`: Forensic Audit phase.
* `VS-000-template.md`: Master markdown blueprint for new stories.
* `VS-000-template.yaml`: Master YAML blueprint for new stories.
